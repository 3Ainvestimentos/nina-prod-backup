...
export function LeaderTrackingContent({
  ...
  const isAuthorized = useMemo(() => {
    ...
    if (!user && !employees) return true; // Simples check de loading
  ...
  const handleSaveQuality = async (notes: QualityIndexNotes) => {
    ...
    const employeeDocRef = doc(firestore, "employees", selectedLeader.id);

    try {
      await addDoc(interactionsCollection, interactionToSave);
      await setDocumentNonBlocking(employeeDocRef, { qualityScore: notes.qualityScore }, { merge: true });
              ...
              <Dialog open={isInteractionDialogOpen} onOpenChange={handleInteractionDialogOpenChange}>
                ...
                <DialogContent className={selectedInteractionType === 'Índice de Qualidade' ? "max-w-3xl" : "sm:max-w-xl"}>
                  ...
                  <DialogFooter>
                    ...
                    <Button 
                      onClick={
                        selectedInteractionType === 'Feedback' ? handleStartInteraction : undefined
                      }
                      type={
                        selectedInteractionType === 'Feedback' ? 'button' : 'submit'
                      }
                      form={
