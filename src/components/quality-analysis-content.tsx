...
export function QualityAnalysisContent({
  ...
  React.useEffect(() => {
    const fetchInteractions = async () => {
      ...
      // Buscar interações apenas dos funcionários selecionados
  ...
  const barChartData = useMemo(() => {
    return selectedEmployees.map(emp => {
      // Tentar pegar do perfil primeiro
      let quality = emp.qualityScore;

      // Se não tiver no perfil, tentar pegar das interações carregadas (fallback para dados legados)
      if (quality === undefined) {
        ...
        quality = latestQuality?.qualityScore ?? 0;
      }
  ...
  const handleSelectHighQuality = async () => {
    if (!managedEmployees) return;
    
    // Lista para armazenar IDs que passam no filtro
    const highQualityIds: string[] = [];
    
    // Identificar funcionários que precisam de verificação no banco (dados legados sem qualityScore no perfil)
    const employeesToCheck = managedEmployees.filter(emp => emp.qualityScore === undefined);
    
    // Adicionar funcionários que já têm score >= 5 no perfil
    managedEmployees.forEach(emp => {
      if (emp.qualityScore !== undefined && emp.qualityScore >= 5) {
        highQualityIds.push(emp.id);
      }
    });

    // Se houver funcionários sem score no perfil, buscar suas últimas interações
    if (employeesToCheck.length > 0) {
      setLoadingInteractions(true); // Mostrar loading durante a verificação
      const { getDocs, query, where } = await import("firebase/firestore");
      
      await Promise.all(employeesToCheck.map(async (emp) => {
        try {
          const interactionsRef = collection(firestore, "employees", emp.id, "interactions");
          // Buscar interações de qualidade
          const q = query(interactionsRef, where("type", "==", "Índice de Qualidade")); 
          const snapshot = await getDocs(q);
          
          if (!snapshot.empty) {
            // Encontrar a mais recente
            const interactions = snapshot.docs.map(d => d.data() as Interaction);
            const latest = interactions.sort((a, b) => new Date(b.date).getTime() - new Date(a.date).getTime())[0];
            
            if (latest && latest.qualityScore !== undefined && latest.qualityScore >= 5) {
              highQualityIds.push(emp.id);
            }
          }
        } catch (err) {
          console.error(`Erro ao verificar qualidade para ${emp.name}:`, err);
        }
      }));
      setLoadingInteractions(false);
    }
